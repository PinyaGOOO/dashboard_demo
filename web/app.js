(() => {
  ...
  function renderBlueprints() {
    ...
    app.innerHTML = `<section class="page">
      ${pageHeader("Сценарии развёртывания", "Версионируемые шаблоны описывают топологию, сеть, bootstrap и правила проверки стенда.", `<button class="button button--primary" data-blueprint-new>${icon("plus")}Новый сценарий</button>`)}
      <div class="blueprint-summary"><div>${icon("code")}<span><strong>${blueprints.filter(item => item.status === "active").length} опубликовано</strong><small>${blueprints.filter(item => item.status === "draft").length} черновик · ${blueprints.reduce((sum, item) => sum + item.vm_count, 0)} VM в каталоге</small></span></div>
  ...
  function blueprintCard(blueprint) {
    ...
    return `<article class="blueprint-card blueprint-card--${tones[blueprint.category] || "slate"}">
      ...
      <div class="blueprint-specs"><span><small>Машины</small><strong>${blueprint.vm_count} VM</strong></span><span><small>Шаблон</small><strong class="mono">${blueprint.template_vmid}</strong></span><span><small>Деплой</small><strong>≈ ${blueprint.estimated_minutes} мин</strong></span></div>
  ...
  function openDeployWizard(preselectedId = null) {
    ...
    const model = {
      ...
      node: "auto", max_participants: 12, ttl_hours: 8, owner: "Администратор",
    };
    const draw = () => {
      const blueprint = available.find(item => item.id === Number(model.blueprint_id)) || available[0];
      const steps = ["Сценарий", "Ресурсы", "Доступ", "Проверка"];
      ...
      if (model.step === 1) {
        body += `<div class="wizard-section"><h3>Выберите сценарий</h3><p>Топология и проверочный код будут закреплены за новым стендом.</p><div class="scenario-picker">${available.map(item => `<button type="button" class="scenario-option ${item.id === Number(model.blueprint_id) ? "is-selected" : ""}" data-wizard-blueprint="${item.id}"><span>${icon(item.category === "Безопасность" ? "shield" : "server")}</span><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.code)} · ${item.vm_count} VM · ≈ ${item.estimated_minutes} мин</small></div><i>${icon("check")}</i></button>`).join("")}</div>
        ...
        body += `<div class="wizard-section"><h3>Размещение и вместимость</h3><p>По умолчанию VM распределяются с наименее загруженной ноды.</p><div class="deploy-plan-card"><div class="deploy-plan-card__icon">${icon("server")}</div><div><strong>${escapeHtml(blueprint.name)}</strong><small>${blueprint.vm_count} VM · ${blueprint.clone_type === "full" ? "полные клоны" : "связанные клоны"}</small></div><span>${escapeHtml(blueprint.storage || "из шаблона")}</span></div>
          <div class="form-grid"><label class="field"><span class="field-label">Политика размещения</span><select class="input" id="deploy-node"><option value="auto" ${model.node === "auto" ? "selected" : ""}>Автоматически по нагрузке</option>${state.data.metrics.nodes.filter(node => node.status === "online").map(node => `<option value="${escapeHtml(node.name)}" ${model.node === node.name ? "selected" : ""}>${escapeHtml(node.name)} · CPU ${formatNumber(node.cpu)}% · RAM ${formatNumber(node.ram)}%</option>`).join("")}</select></label><label class="field"><span class="field-label">Максимум участников</span><input class="input" id="deploy-capacity" type="number" min="1" max="100" value="${model.max_participants}"></label></div>
          <div class="capacity-preview"><div><span>Сеть</span><strong class="mono">${escapeHtml(blueprint.subnet || "DHCP")}</strong></div><div><span>Bridge</span><strong class="mono">${escapeHtml(blueprint.bridge)}</strong></div><div><span>Шаблон VMID</span><strong class="mono">${blueprint.template_vmid}</strong></div><div><span>Оценка</span><strong>${blueprint.estimated_minutes} мин</strong></div></div></div>`;
        ...
        body += `<div class="wizard-section"><div class="confirm-hero"><span>${icon("check")}</span><h3>План готов к запуску</h3><p>Проверьте параметры. Развёртывание продолжится в фоне.</p></div><dl class="review-list"><div><dt>Стенд</dt><dd><strong>${escapeHtml(model.name)}</strong><small class="mono">${escapeHtml(model.pool_id)}</small></dd></div><div><dt>Сценарий</dt><dd><strong>${escapeHtml(blueprint.name)}</strong><small>${escapeHtml(blueprint.code)} · v${escapeHtml(blueprint.version)}</small></dd></div><div><dt>Топология</dt><dd><strong>${blueprint.vm_count} VM · ${blueprint.clone_type === "full" ? "Full clone" : "Linked clone"}</strong><small>${escapeHtml(model.node === "auto" ? "Автораспределение" : model.node)}</small></dd></div><div><dt>Сеть</dt><dd><strong class="mono">${escapeHtml(blueprint.subnet || "DHCP")}</strong><small class="mono">${escapeHtml(blueprint.bridge)}</small></dd></div><div><dt>Доступ</dt><dd><strong>${model.max_participants} участников</strong><small>${escapeHtml(model.owner)} · ${model.ttl_hours} ч</small></dd></div></dl>
  ...
  function openBlueprintEditor(id = null) {
    ...
    const blueprint = original || {
      ...
      vm_count: 1, template_vmid: 0, clone_type: "full", storage: "", bridge: "vmbr0", subnet: "",
      estimated_minutes: 8, tags: [], deploy_script: "#!/usr/bin/env bash\nset -euo pipefail\n\n# Подготовка гостевой VM\n",
    };
    const body = `<form id="blueprint-form" class="blueprint-form"><div class="form-section"><h3>Основные сведения</h3><div class="form-grid">
      ...
      <div class="form-section"><h3>Топология Proxmox</h3><div class="form-grid form-grid--four"><label class="field"><span class="field-label">VMID шаблона</span><input class="input mono" type="number" name="template_vmid" value="${blueprint.template_vmid}" min="0"></label><label class="field"><span class="field-label">Количество VM</span><input class="input" type="number" name="vm_count" value="${blueprint.vm_count}" min="1" max="50"></label><label class="field"><span class="field-label">Тип клона</span><select class="input" name="clone_type"><option value="full" ${blueprint.clone_type === "full" ? "selected" : ""}>Full clone</option><option value="linked" ${blueprint.clone_type === "linked" ? "selected" : ""}>Linked clone</option></select></label><label class="field"><span class="field-label">Оценка, мин</span><input class="input" type="number" name="estimated_minutes" value="${blueprint.estimated_minutes}" min="1"></label>
      <label class="field"><span class="field-label">Storage</span><input class="input mono" name="storage" value="${escapeHtml(blueprint.storage)}" placeholder="из шаблона"></label><label class="field"><span class="field-label">Bridge</span><input class="input mono" name="bridge" value="${escapeHtml(blueprint.bridge)}"></label><label class="field field--span-2"><span class="field-label">Подсеть</span><input class="input mono" name="subnet" value="${escapeHtml(blueprint.subnet)}" placeholder="10.39.10.0/24"></label></div></div>
